...
{
	 /// <summary>
	 /// FilterLoader finds the dll and ClassID of the COM object responsible for filtering a
	 /// specific file extension. It then loads that dll, creates the appropriate COM object and
	 /// returns a pointer to an IFilter instance
	 /// </summary>
	 public static class FilterLoader
	 {
		  private class DestructorObj
		  {
				~DestructorObj()
				{
					 ReleaseClassCache();
		  ...
		  private class LibHandle : SafeHandle
		  {
		  ...
		  private static readonly DestructorObj _obj = new DestructorObj();

		  //SqlString is a real nullable string supported by Microsoft, sigh...
		  private static readonly Dictionary<SqlString, KeyValuePair<LibHandle, IntPtr>> Cache_V1 = new Dictionary<SqlString, KeyValuePair<LibHandle, IntPtr>>();

		  private static readonly Dictionary<SqlString, Type> Cache_V2 = new Dictionary<SqlString, Type>();

		  private delegate int DllCanUnloadNow();

		  //DllGetClassObject fuction pointer signature
		  private delegate int DllGetClassObject(ref Guid ClassId, ref Guid InterfaceId, out IntPtr ppunk);

		  /// <summary>
		  /// </summary>
		  /// <param name="ext"></param>
		  /// <returns></returns>
		  public static IFilter LoadIFilter(string ext)
		  {
		  ...
		  /// <summary>
		  /// Firstly, this method try to load OLE stream from given binary array. If failed, then
		  /// default filter or Null filter will be last choice.
		  /// </summary>
		  /// <param name="bytes"></param>
		  ...
		  public static void ReleaseClassCache()
		  {
				foreach (var pair in Cache_V1)
				{
					 ...
					 if (dllCanUnloadNowPtr != IntPtr.Zero)
					 {
						  ...
						  if (dllCanUnloadNow() != COMTypeDef.Constants.S_OK)
								continue;
					 }
		  ...
		  private static extern int BindIFilterFromStorage(IStorage pStg, IntPtr pUnkOuter, out IFilter ppIUnk);

		  [DllImport("query.dll", PreserveSig = false)]
		  private static extern int BindIFilterFromStream(IStream pStm, IntPtr pUnkOuter, out IFilter ppIUnk);

		  [DllImport("ole32.dll", PreserveSig = false)]
		  private static extern ILockBytes CreateILockBytesOnHGlobal(HandleRef hGlobal, bool fDeleteOnRelease);

		  // The BindIFilterFromStream function takes an IStream interface pointer of a Structured
		  // Storage object. Be sure not to create a simple stream (look into STATSTG) to feed this
		  // function. It works fine.
		  [SecurityCritical, DllImport("ole32.dll", CharSet = CharSet.Auto, ExactSpelling = true, PreserveSig = false)]
		  private static extern void CreateStreamOnHGlobal(IntPtr hGlobal, bool fDeleteOnRelease, out IStream ppstm);
		  ...
		  private static IntPtr GetClassFactoryFromDll(string ext, string dllName, string filterPersistClass)
		  {
				...
				var IClassFactoryGUID = new Guid(COMTypeDef.Constants.IClassFactoryGUID); //IClassFactory class id
				IntPtr unk;
		  ...
		  {
				if (ext == "*")
					 return COMTypeDef.Constants.PH_IDefaultFilter;
				if (ext == null)
					 return COMTypeDef.Constants.PH_INullFilter;
		  ...
		  private static IFilter InitIFilter(IFilter filter)
		  {
				...
				if (returnCodes == COMTypeDef.Constants.S_OK)
					 return filter;
		  ...
		  private static IFilter LoadFilterFromDll(string ext, string dllName, string filterPersistClass)
		  {
				...
				else
				{
					 ...
					 if (null == classFactory)
						  return null;
					 var IFilterGUID = new Guid(COMTypeDef.Constants.IFilterGUID);
		  ...
		  private static extern int LoadIFilter(string pwcsPath, [MarshalAs(UnmanagedType.IUnknown)] object pUnkOuter, out IFilter ppIUnk);

		  [DllImport("kernel32.dll")]
		  private static extern LibHandle LoadLibrary(string lpFileName);
		  ...
		  [DllImport("ole32.dll", PreserveSig = false)]
		  private static extern IStorage StgOpenStorageOnILockBytes(ILockBytes iLockBytes, IStorage pStgPriority, int grfMode, int sndExcluded, int reserved);
	 }
}